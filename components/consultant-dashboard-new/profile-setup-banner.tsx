...
export function ProfileSetupBanner({
  ...
  return (
    <div className="w-full mb-4 rounded-xl bg-brand-blue px-6 md:px-8 py-4 md:py-5">
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div className="flex-1">
              ...
              <p className="text-white/90 text-xs">{getDescriptionText()}</p>
            </div>
            ...
            {!isComplete && (
              <Link
                ...
                className="flex items-center justify-center gap-2 px-4 py-2 mt-0 sm:-mt-3 bg-white text-black font-semibold rounded-xl hover:bg-slate-50 transition-colors whitespace-nowrap text-sm"
              >
                Complete Profile
                <svg
          ...
          <div className="flex-1 flex gap-1 h-1">
            {[0, 1, 2, 3].map((i) => {
              const filled = Math.round(
                (Math.min(completionPercentage, 100) / 100) * 4,
              );
