...
export const ProjectsCard: React.FC<{ data: DashboardData }> = ({ data }) => {
  return (
    <div className="rounded-lg p-4 border border-[#E5E5E5] md:bg-transparent bg-white shadow-custom">
      ...
      <div className="flex items-center gap-2 mb-4">
        <div className="flex flex-1 flex-col justify-start items-start bg-brand-pink rounded-xl px-4 py-2 border border-[#E5E5E5]">
          ...
          <span className="text-xl font-neue text-gray-900">{data?.projects?.total_projects || 0}</span>
        </div>
        <div className="flex flex-1 flex-col justify-start items-start bg-brand-pink rounded-xl px-4 py-2 border border-[#E5E5E5]">
          <span className="text-xs text-gray-600 font-manrope">Active Projects</span>
          <span className="text-xl font-neue text-gray-900">{data?.projects?.active || 0}</span>
        </div>
      </div>
